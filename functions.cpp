...
uint16_t ip_checksum(uint8_t * ip_packet) {
	...
	iph->checksum = 0;
	uint16_t checksum = calc((uint16_t *)iph, ((iph->ver_hl) & 0x0f) * 4);
...
uint16_t tcp_checksum(uint8_t * ip_packet, int len) {
	...
	memcpy(&pseh.src, &iph->ip_src, sizeof(pseh.src));
	memcpy(&pseh.dest, &iph->ip_dst, sizeof(pseh.dest));
	pseh.protocol = iph->p;
	pseh.tcp_len = htons(len - (((iph->ver_hl) & 0x0f) * 4));
